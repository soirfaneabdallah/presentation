...
\only<2>{
\textcolor{orange}{\textbf{\faIcon{chart-line} Afrique Subsaharienne}}
\begin{itemize}
    \item Croissance \textbf{rapide} des déchets
    \item \textbf{90\%} en décharge sauvage
    \item \textbf{57\%} de déchets organiques
\end{itemize}
}
...
\only<2>{
\begin{block}{Trois Approches Explorées}
\begin{itemize}
    \item \textbf{Confiance du modèle} : Seuil sur les probabilités de prédiction
...
\begin{itemize}
    ...
    \item Données inconnues : batteries, couches, cartons, autres déchets
...
\end{frame}
\begin{frame}{Détection par Seuil de Confiance}

\only<1>{
\begin{block}{Principe de la Méthode}
\begin{itemize}
    \item Analyse des probabilités de prédiction du modèle
    \item Rejet si probabilité maximale $<$ seuil fixé
    \item Acceptation si probabilité maximale $\geq$ seuil
    \item 20 valeurs testées entre 50\% et 100\%
\end{itemize}
\end{block}

\begin{center}
\includegraphics[width=0.8\linewidth]{detect.png}
\end{center}
}

\only<2>{
\begin{block}{Résultats Expérimentaux}
\begin{center}
\begin{tabular}{lcc}
\hline
\textbf{Modèle} & \textbf{Meilleur seuil} & \textbf{Exactitude} \\
\hline
Sans TL (Flatten) & 0.973 & 76.79\% \\
Sans TL (AvgPooling) & 0.999 & 75.89\% \\
Avec TL (Flatten) & 0.973 & 72.27\% \\
Avec TL (AvgPooling) & 0.973 & 72.64\% \\
\hline
\end{tabular}
\end{center}
\end{block}
...
\begin{itemize}
    \item Courbe montre l'évolution performance/seuil
    \item Meilleurs résultats sans transfert learning
    \item Seuils optimaux entre 0.973 et 0.999
    \item Performance limitée à $\sim$77\% maximum
\end{itemize}
\end{alertblock}
}
...
\begin{center}
\includegraphics[width=0.6\linewidth]{train_auto.png}
\includegraphics[width=0.6\linewidth]{test_auto.png}
\end{center}
\begin{center}
...
\end{frame}
\begin{frame}{Détection par Réseaux Antagonistes GANs}

\only<1>{
\begin{block}{Principe et Architecture}
\begin{itemize}
    \item Deux réseaux: Générateur (crée des images) vs Discriminateur (détecte les fausses)
    \item Objectif: Exploiter le discriminateur pour détecter les anomalies
    \item Images taille réduite: 64×64×3 pour contraintes matérielles
\end{itemize}
\end{block}

\begin{center}
\includegraphics[width=0.8\linewidth]{gan_loss.png}
\end{center}
\begin{center}
\small Évolution instable des pertes - Pas de convergence
\end{center}
}

\only<2>{
\begin{block}{Résultats et Limitations}
\begin{center}
\includegraphics[width=0.8\linewidth]{gan_ge.png}
\end{center}
\begin{center}
\small Images générées de mauvaise qualité - Formes floues
\end{center}
\end{block}
}
\only<3>{
\begin{alertblock}{Échec de l'Entraînement}
\begin{itemize}
    \item Arrêt après 58 époques (pas d'amélioration)
    \item Discriminateur trop dominant
    \item Impossible de tester la détection d'anomalies
    \item Approche prometteuse mais nécessite plus de ressources
\end{itemize}
\end{alertblock}
}
...
Mise en place d'un bras robotique pour déployer le système en conditions réelles.
\end{alertblock}
